...
module.exports = {
    ...
    article_publish(req, res) {
        ...
        if (!req.file) {
            ...
            req.file.size > 1024 * 1024 * 5 || ['image/gif', 'image/png', 'image/jpeg'].indexOf(req.file.mimetype) == -1
        ) {
            res.send({
                msg: '文件大小或类型不对，请检查',
                code: 400
    ...
    article_edit(req, res) {
        ...
        if (req.file) {
            if (
                req.file.size > 1024 * 1024 || ['image/gif', 'image/png', 'image/jpeg'].indexOf(req.file.mimetype) ==
                -1
            ) {
                res.send({
                    msg: '文件大小或类型不对，请检查',
    ...
    userinfo_edit(req, res) {
        ...
        if (req.file) {
            // 文件大小判断
            if (
                req.file.size > 2048 * 2048 || ['image/gif', 'image/png', 'image/jpeg'].indexOf(req.file.mimetype) ==
                -1
            ) {
                res.send({
                    msg: '文件大小或类型不对，请检查',
